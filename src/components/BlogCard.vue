<template>
  <RouterLink :to="`/blog/${blog.slug}`" class="block transition-all duration-300 hover:scale-105 focus:outline-none focus:ring-2 focus:ring-blue-500 rounded-xl">
    <BaseCard hover class="h-full">
      <template #image>
        <div class="relative bg-gray-100">
          ...
          <!-- Featured Badge -->
          <div v-if="blog.is_featured" class="absolute top-2 left-2 sm:top-3 sm:left-3">
            <span class="inline-flex items-center px-2 py-1 sm:px-2.5 sm:py-0.5 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800 backdrop-blur-sm">
          ...
          <div class="absolute top-2 right-2 sm:top-3 sm:right-3">
            <span class="inline-flex items-center px-2 py-1 sm:px-2.5 sm:py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800 backdrop-blur-sm">
              {{ blog.category }}
            </span>
          </div>
        </div>
      ...
      <div class="space-y-2 sm:space-y-3 flex flex-col flex-1 p-1">
        <div class="flex-1">
          ...
          <div class="flex items-center mt-1 mb-2">
            <div class="flex items-center">
              ...
              />
              <svg v-else class="w-4 h-4 text-gray-500 mr-2 flex-shrink-0" fill="currentColor" viewBox="0 0 24 24">
                <path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"/>
              </svg>
              <span class="text-sm text-gray-600 truncate">{{ blog.author_name }}</span>
            </div>
          ...
          <!-- Tags -->
          <div v-if="blog.tags && blog.tags.length > 0" class="flex items-center mt-2 gap-1 flex-wrap">
            <span 
        ...
        <div class="pt-3 border-t border-gray-200 mt-auto">
          <BaseButton variant="outline" size="md" class="w-full text-sm sm:text-base">
            Leer Artículo
          </BaseButton>
        </div>
      </div>
...
if (import.meta.env.DEV && props.blog.featured_image_url) {
  console.log('BlogCard: Found image for blog', props.blog.id, props.blog.featured_image_url)
}
...
const blogImageUrl = computed(() => {
  ...
  if (import.meta.env.DEV && imageUrl.includes('app.tierrasonada.com')) {
    return imageUrl.replace('https://app.tierrasonada.com', '').replace('http://app.tierrasonada.com', '')
  }
