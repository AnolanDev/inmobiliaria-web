<template>
  <RouterLink :to="`/proyectos/${project.id}`" class="block transition-all duration-300 hover:scale-105 focus:outline-none focus:ring-2 focus:ring-primary-500 rounded-xl">
      ...
      <template #image>
        <div class="relative">
          ...
          <div class="absolute top-2 left-2 sm:top-3 sm:left-3">
            <span :class="getTypeColorClass(project.type)" class="inline-flex items-center px-2 py-1 sm:px-2.5 sm:py-0.5 rounded-full text-xs font-medium backdrop-blur-sm">
              {{ project.type }}
            </span>
          </div>
          <div class="absolute top-2 right-2 sm:top-3 sm:right-3">
            <span :class="getStatusColorClass(project.status)" class="inline-flex items-center px-2 py-1 sm:px-2.5 sm:py-0.5 rounded-full text-xs font-medium backdrop-blur-sm">
              {{ project.status }}
            </span>
          </div>
        </div>
    ...
    <div class="space-y-3 flex flex-col flex-1">
      <div class="flex-1">
        ...
        <div class="flex items-center mt-1 mb-2" v-if="project.city || project.state || project.location">
          <svg class="w-4 h-4 text-gray-500 mr-1 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div class="flex items-center justify-between text-sm text-gray-600 flex-wrap gap-2">
        <div class="flex items-center space-x-1 min-w-0">
          ...
          <span class="truncate">{{ project.property_count }} propiedades</span>
        </div>
