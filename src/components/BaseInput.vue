<template>
  <div>
    <label
      v-if="label"
      :for="id"
      class="block text-sm sm:text-base font-medium text-gray-700 mb-2 leading-tight"
    >
      {{ label }}
    ...
    <p v-if="error" class="mt-2 text-sm text-red-600 leading-relaxed">
      {{ error }}
    </p>
    <p v-else-if="hint" class="mt-2 text-sm text-gray-500 leading-relaxed">
      {{ hint }}
    </p>
  </div>
</template>
...
const inputClasses = computed(() => {
  ...
  const mobile =
    "text-base sm:text-sm min-h-[48px] -webkit-tap-highlight-color: transparent;";
